...
class Eikonal_3D(torch.nn.Module):
    def __init__(
        self,
        m,
        n,
    ...
    def init_u(self,ix,iy,iz,phase):
        ...
        l = self.l
        nl = n*l
        ixu = int(np.ceil(ix)) ; ixd = int(np.floor(ix))
        iyu = int(np.ceil(iy)) ; iyd = int(np.floor(iy))
        izu = int(np.ceil(iz)) ; izd = int(np.floor(iz))

        if phase == 'P':
            self.u0 = torch.ones(self.m * self.n *self.l,dtype = torch.double) * 1000.0
            self.u0[ixu*nl +iyu*l +izu] = np.sqrt((ix-ixu)**2+(iy-iyu)**2+(iz-izu)**2)*h/self.f[ixu*nl +iyu*l +izu]
            self.u0[ixu*nl +iyu*l +izd] = np.sqrt((ix-ixu)**2+(iy-iyu)**2+(iz-izd)**2)*h/self.f[ixu*nl +iyu*l +izd]
            self.u0[ixu*nl +iyd*l +izu] = np.sqrt((ix-ixu)**2+(iy-iyd)**2+(iz-izu)**2)*h/self.f[ixu*nl +iyd*l +izu]
            self.u0[ixu*nl +iyd*l +izd] = np.sqrt((ix-ixu)**2+(iy-iyd)**2+(iz-izd)**2)*h/self.f[ixu*nl +iyd*l +izd]
            self.u0[ixd*nl +iyu*l +izu] = np.sqrt((ix-ixd)**2+(iy-iyu)**2+(iz-izu)**2)*h/self.f[ixd*nl +iyu*l +izu]
            self.u0[ixd*nl +iyu*l +izd] = np.sqrt((ix-ixd)**2+(iy-iyu)**2+(iz-izd)**2)*h/self.f[ixd*nl +iyu*l +izd]
            self.u0[ixd*nl +iyd*l +izu] = np.sqrt((ix-ixd)**2+(iy-iyd)**2+(iz-izu)**2)*h/self.f[ixd*nl +iyd*l +izu]
            self.u0[ixd*nl +iyd*l +izd] = np.sqrt((ix-ixd)**2+(iy-iyd)**2+(iz-izd)**2)*h/self.f[ixd*nl +iyd*l +izd]

        elif phase == 'S':
            self.u0 = torch.ones(self.m * self.n *self.l,dtype = torch.double) * 1000.0
            self.u0[ixu*nl +iyu*l +izu] = np.sqrt((ix-ixu)**2+(iy-iyu)**2+(iz-izu)**2)*h/(self.f[ixu*nl +iyu*l +izu]*self.ratio_s[ixu*nl + iyu*l + izu])
            self.u0[ixu*nl +iyu*l +izd] = np.sqrt((ix-ixu)**2+(iy-iyu)**2+(iz-izd)**2)*h/(self.f[ixu*nl +iyu*l +izd]*self.ratio_s[ixu*nl + iyu*l + izd])
            self.u0[ixu*nl +iyd*l +izu] = np.sqrt((ix-ixu)**2+(iy-iyd)**2+(iz-izu)**2)*h/(self.f[ixu*nl +iyd*l +izu]*self.ratio_s[ixu*nl + iyd*l + izu])
            self.u0[ixu*nl +iyd*l +izd] = np.sqrt((ix-ixu)**2+(iy-iyd)**2+(iz-izd)**2)*h/(self.f[ixu*nl +iyd*l +izd]*self.ratio_s[ixu*nl + iyd*l + izd])
            self.u0[ixd*nl +iyu*l +izu] = np.sqrt((ix-ixd)**2+(iy-iyu)**2+(iz-izu)**2)*h/(self.f[ixd*nl +iyu*l +izu]*self.ratio_s[ixd*nl + iyu*l + izu])
            self.u0[ixd*nl +iyu*l +izd] = np.sqrt((ix-ixd)**2+(iy-iyu)**2+(iz-izd)**2)*h/(self.f[ixd*nl +iyu*l +izd]*self.ratio_s[ixd*nl + iyu*l + izd])
            self.u0[ixd*nl +iyd*l +izu] = np.sqrt((ix-ixd)**2+(iy-iyd)**2+(iz-izu)**2)*h/(self.f[ixd*nl +iyd*l +izu]*self.ratio_s[ixd*nl + iyd*l + izu])
            self.u0[ixd*nl +iyd*l +izd] = np.sqrt((ix-ixd)**2+(iy-iyd)**2+(iz-izd)**2)*h/(self.f[ixd*nl +iyd*l +izd]*self.ratio_s[ixd*nl + iyd*l + izd])
        else:
    ...
    def forward(self,x,y,z,g_x,g_y,g_z,phase):
        ...
        if phase == 'P':
            ...
            self.u = Eik3DFunction.apply(u,self.u0,self.m,self.n,self.l,self.h,self.tol,True,1/(self.f*self.ratio_s))
        else:
            exit('S . T')
        ...
        t_xyz =  (z2-grid_z)*(y2-grid_y)*((x2-grid_x)*f111_ + (grid_x-x1)*f211_) +\
